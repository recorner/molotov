// CommonJS PM2 config (package.json has "type":"module", so PM2 needs a .cjs file).
// Keep in sync with ecosystem.config.js.
module.exports = {
  apps: [{
    name: 'molotov',
    script: './bot.js',
    cwd: __dirname,   // apps/bot
    instances: 1, // Single instance for Telegram bots (one polling client per token)
