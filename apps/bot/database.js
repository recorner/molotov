...
// The bot and the web app (apps/web) share one Postgres database. `pgCompat`
// speaks the sqlite3 API this file was written against, so the call sites below
// — and across handlers/ and utils/ — keep working while they migrate to Prisma.
...
if (DRIVER === 'sqlite') {
  ...
  db = new sqlite3.Database('./store.db', (err) => {
    if (err) console.error('[DB] Error:', err.message);
    else console.log('[DB] SQLite store initialized (legacy driver).');
  });
} else {
  db = (await import('./db/pgCompat.js')).default;
  console.log('[DB] Postgres store initialized (shared with web app).');
}
